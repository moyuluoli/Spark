...
object SparkKafkaDemo {
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("cw").setMaster("local[*]")
...
//    val topics = Array("spark_kafka")
    val topics = Array("tbox_period_60s")
    //2.使用KafkaUtil连接Kafak获取数据
    //注意:
    //如果MySQL中没有记录offset,则直接连接,从latest开始消费
    //如果MySQL中有记录offset,则应该从该offset处开始消费
    val offsetMap: mutable.Map[TopicPartition, Long] = OffsetUtil.getOffsetMap("SparkKafkaDemo","tbox_period_60s")
//    val offsetMap: mutable.Map[TopicPartition, Long] = OffsetUtil.getOffsetMap("SparkKafkaDemo","spark_kafka")
  ...
  object OffsetUtil {
    ...
    def getOffsetMap(groupid: String, topic: String) = {
      val connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/flink?characterEncoding=UTF-8", "root", "root")
